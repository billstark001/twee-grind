...
*/
function escape(str: string | Obj<string>) {
...
const
	...
	*/
	...
	/*d:
		...
		Example usage:
		```
		(change:?passage, (text-size:0.6em))
		|==
	...
	*/
	...
	/*d:
		...
		*link text*, which the player clicks on, and a *passage name* to send the player to.
		...
		Example usage:
		```
		[[Go to the cellar->Cellar]] --Link that goes to a passage named "Cellar".
		[[Parachuting<-Jump]] --Link that goes to a passage named "Parachuting".
		[[Down the hatch]] --Link that goes to a passage named "Down the hatch".
		```
	...
	*/
	...
	/*d:
		...
		Furthermore, if the variable contains a changer, such as that created by (text-style:) and such,
		then the variable can be attached to a hook to apply the changer to the hook:

		```
		(set: $robotText to (font:"Courier New"), _assistantText to (size:19px))
		...
		**Note 3:** In Harlowe 4, If you use a story-wide variable that doesn't exist (that is, it hasn't been created via (set:), (put:), and so forth), an error
		will be produced. This differs from Harlowe 3.
	...
	*/
	...
	/*
		This must open with optional whitespace, because the 'property' lexing rule has a constraint forcing it to be before valid property-indexable tokens,
		and whitespace tokens shouldn't be between it and them.
		This allows, e.g. `$a 's 1st` to be valid.
	*/
	property          = `${ws}'s${mws}${notBefore(`_`)}(${validPropertyName})`,
	...
	*/
	...
	/*
		Identifiers: either "it", "time", "visit", "visits", "exit", "exits", "user", or "pos".
		"it" is a bit of a problem because its possessive is "its", not "it's",
		so we can't use a derivation similar to property.
	*/
	identifier          = either(`it`,`time`,`turns?`,`visits?`,`exits?`,`user`,`pos`) + wb,
...
const Patterns = {
	...
	/*d:
		...
		Comments are used to exclude, temporarily or permanently, certain parts of the code or prose, so that Harlowe ignores it and treats it as if
		it wasn't there. Place a `--` (double hyphen) mark to "comment out" the very next code element or line of prose.
		
		Example usage:
		```
		--[
			Player's loyalty to the Crown.
			Affects odds of defection opportunities.
		]
		(set: $loyalty to --2 6) --[The number 2 is commented out, so Harlowe treats this as (set: $loyalty to 6)]
		--Go to different passages based on selected difficulty.
		(goto: (cond:--[The modeSelect passage controls this variable.] $easyMode, "Easy Begin", "Begin"))
		```

		Rationale:

		Comments can be used to remove certain macro calls or data values without deleting them, in case you feel you might need them later,
		or want to test your story with their absence (for instance, commenting out a (go-to:) call to prevent moving to the instructed passage).
		
		More importantly, comments can be used for conveniently including private, authorial notes about parts of a passage, within the passage itself.
		Quick notes or to-dos about the direction of the story, reminders about why a description or line of dialogue is written a certain way,
		summaries about what a certain block of macros is supposed to do and what the intended purpose of it is, and other such marginalia can be
		included as single commented-out sentences.

		NOTE: Slightly diverging from other programming languages' "line comment" syntax, `--` only comments out non-code prose up to *the next non-prose element*.
		Usually this is a line break, but if there's a macro or macro call before that, then the comment will end before it, and it will
		still be present in the passage. In the line of code `--This is commented out (print:"But this isn't!")`, the (print:) call is still in the passage.
		Pay attention to the syntax highlighting of the passage to determine what exactly a comment comprises.

		Comment hooks:

		If you want to include a multi-line run of explanatory text, or comment out a large segment of code without needing to use a large amount of `--` marks,
		use a **comment hook**. This is simply an entire untagged hook that has been commented out - a combination of the comment `--` and hook `[` `]` syntax -
		but you can think of it as a special kind of comment that begins with `--[` and only ends with a matching `]`.

		Comment hooks also allow explanatory notes to be included *inside* macro calls - trying to place commented-out prose inside macro calls does *not* work
		because plenty of English words are used as keywords in Harlowe, such as `of`, `each` and `to`. Wrapping that prose in a hook (which inside macro calls is
		considered a "code hook") overcomes this issue easily.

		NOTE: Slightly diverging from other programming languages' "block comment" syntax, comment hooks are capable of *nesting*. If you have a block of macro calls
		interspersed with explanatory comment hooks, then it is possible to fully enclose that block, and those comment hooks, inside another larger comment hook
		to comment it out entirely, *without* the inner comment hook's terminating `]` interfering with the syntax:

		```
		--[
		(set: $nextPlanet to (open-storylets: )'s 1st)
		--[$planetCommonNames is a datamap linking storylet passage names to
		in-story names for the planets.]
		Next stop: (print: $nextPlanet of $planetCommonNames). 5… 4… 3… 2… 1…
		(link-goto:"Blastoff", $nextPlanet)
		]
		```

		Incidentally, as part of Harlowe's support for HTML tags, HTML comments using `<!--` and `-->` are also supported. You can use both kinds of comments in your passages, but
		HTML comments cannot be used inside macro calls.

		IMPORTANT: Harlowe does not remove comments from passage code as it exports the finished story HTML file, so any text in comments can be seen by
		a determined player examining the HTML file closely. Be careful not to disclose any personal information in comments, and assume all comment text is
		publicly accessible!!
	...
	*/
	...
	/*d:
		...
		Finally, you can also include HTML comments `<!-- Comment -->` in your prose, although Harlowe 4.0 and later comes with
		its own commenting syntax. See the Comment article for details.
	...
	*/
	scriptStyleTag: `<(${either(`script`,`style`,`textarea`)})${tag.attrs}>`
		+ `[^]*?` + `<\\/\\1>`,
	...
	*/
	...
	/*
		This measure type includes every Measure unit (px, em, rem, w, h, lh) plus time (s, ms). It also allows negative and fractional values.
	*/
	cssMeasure: `(\\-?\\d+\\.?\\d*|\\d*\\.?\\d+)(m?s|px|r?em|w|l?h)${wb}`,
	...
	// Data types
	/*
		This MUST line up with every type in datatypes.ts.
	*/
	datatype: either(
		`alnum`, `alphanumeric`, `any(?:case)?`, `array`, `bool(?:ean)?`, `changer`, `codehook`, `colou?r`, `const`,
		`command`, `dm`, `data${either(`map`,`type`,`set`)}`, `ds`, `digit`,
		`gradient`, `empty`, `even`, `image`, `int${notBefore(`o`)}(?:eger)?`, `lambda`, `lowercase`, `macro`, `measure`, `linebreak`, `newline`,
