import { ConditionalExcept, SetOptional, SetRequired } from "type-fest"
// Because this file is used by both TwineMarkup and Harlowe,
// these can't be an import from either target's Utils.
type Obj<T> = Record<string, T>
type ExceptMethods<T> = ConditionalExcept<T, (...args:any[]) => void>
/*
...
/*
	Lexer rules, used to tokenise input.
	This is generic so that token subclasses or variants can be used with it.
*/
type Rule<T> = {
	fn: (match:string | RegExpMatchArray) => T
	cannotFollowText?: boolean
	constraint?: (prev: T | null) => boolean
} & ({
	/*
		Pattern is optional because most rules have automatically generated patterns based on their name.
	*/
	pattern?: RegExp
	plainCompare?: false
} | {
...
/*
	In this record, all rules are "completed" (their pattern has been inserted), so the pattern
	is no longer optional.
*/
const rules: Obj<SetRequired<Rule<Token>, `pattern`>> = {}

type TokenData = SetOptional<ExceptMethods<Omit<Token, `addChild`>>, `start` | `end` | `children` | `innerText`>

/*
	The Lexer tokens class.
	This just has some basic methods that iterate over tokens' children,
...
class Token {
	...
	declare type: string
	declare text:string
	declare innerMode: string[]
	declare place?: string
	start = 0
	end = 0
	children: Token[] = []
	innerText = ``
	declare matches?: Obj<string>
	declare cannotCross?: string[]
	declare isFront?: boolean
	declare aka?: string
	/*
		Other properties specific to each token are defined in markup.ts.
	*/

	constructor(tokenData: TokenData) {
		assign(this, tokenData)
	}
	...
	*/
	addChild(tokenData: TokenData) {
		const index = this.lastChildEnd() || 0
		...
		*/
		...
		childToken.end = index + tokenData.text?.length || 0
		/*
	...
	*/
	everyLeaf(fn: (t:Token) => boolean): boolean {
	...
	*/
	...
	debugRender(depth:number = 0):string {
		return `${`\t`.repeat(depth)}• ${this.type}\n${this.children.map(t => t.debugRender(depth+1)).join(``)}`
	}
	...
	while(index < endIndex) {
		...
		for (; i < l; i+=1) {
			...
			*/
			...
			/*
				...
				are met...
			*/
			const tokenData = rule.fn(rule.plainCompare ? rule.pattern : (
				rule.pattern.exec(slice) || ``))
			/*
				...
				for(; frontTokenStack && ft < frontTokenStack.length; ft += 1) {
					...
					*/
					...
					/*
						...
						macroFront.
					*/
					if (tokenData.cannotCross?.includes(type)) {
		...
		*/
		if (i === l) {
			index += 1
			if (lastToken === null) {
				lastToken = new Token({ type: `text`, text: ``, innerMode: mode })
			}
		}
	}
...
function foldTokens(parentToken: Token, backToken: Token, frontToken: Token) {
	...
	*/
	if (!backToken.matches?.[frontToken.type]) {
		throw Error(`Couldn't fold backToken ${backToken.type} to match frontToken ${frontToken.type}`)
	}
	...
	*/
	for (let key in frontToken) {
		if(key in frontToken && !(key in backToken)) {
			type T = Obj<unknown>
			;(<T><unknown>backToken)[key] = (<T><unknown>frontToken)[key]
		}
	}
	/*
...
const Lexer = {
	...
	*/
	lex(src: string, place = ``, innerMode = `start`, flat = false) {
